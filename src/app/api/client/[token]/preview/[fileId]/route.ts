...
export async function GET(_request: NextRequest, { params }: { params: Promise<{ token: string; fileId: string }> }) {
  try {
    ...
    const { data, error } = await db.storage.from("assignment-files").createSignedUrl(file.storage_path, 60);
    if (error || !data) throw error || new Error("Unable to open preview.");
