...
function DashboardView({ token, data, busy, setBusy, post, reload, setNotice }: any) {
  ...
  return <div className="portal-grid">
    ...
    <aside className="stack">
      ...
      {accepted ? <section className="bank-box"><div className="panel-title"><h3>Payment details</h3><Banknote/></div><div className="amount">{formatMoney(assignment.quoted_amount, assignment.currency || data.settings.currency)}</div><div className="detail-list"><div className="detail-row"><span>Bank</span><strong>{data.settings.bank_name}</strong></div><div className="detail-row"><span>Account name</span><strong>{data.settings.account_name}</strong></div><div className="detail-row"><span>Account number</span><strong>{data.settings.account_number}</strong></div><div className="detail-row"><span>Branch</span><strong>{data.settings.bank_branch}</strong></div>{data.settings.bank_code ? <div className="detail-row"><span>Bank code</span><strong>{data.settings.bank_code}</strong></div> : null}{data.settings.branch_code ? <div className="detail-row"><span>Branch code</span><strong>{data.settings.branch_code}</strong></div> : null}{data.settings.swift_code ? <div className="detail-row"><span>SWIFT / BIC</span><strong>{data.settings.swift_code}</strong></div> : null}{data.settings.account_type ? <div className="detail-row"><span>Account type</span><strong>{data.settings.account_type}</strong></div> : null}</div><p className="small" style={{color:"#c6d6e8", marginTop:14}}>{data.settings.payment_note}</p></section> : null}
